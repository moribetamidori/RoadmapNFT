 ...
 */
async function main() {
  const RoadmapsConstants = await hre.ethers.getContractFactory(
    "RoadmapsConstants"
  );
  const constants = await RoadmapsConstants.deploy();
  await constants.deployed();

  const Roadmaps = await hre.ethers.getContractFactory("Roadmaps", {
    libraries: {
      RoadmapsConstants: constants.address,
    },
  });
  const roadmaps = await Roadmaps.deploy(
    MAX_SUPPLY // maxSupply
  );
  await roadmaps.deployed();
  console.log(`Floot deployed to: ${roadmaps.address}`);
  console.log(`Constants deployed to: ${constants.address}`);
}
